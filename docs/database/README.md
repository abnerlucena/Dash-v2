...
> **Estado atual do schema:** `v0.15.0` — implementado no Supabase (projeto de testes). A fábrica real de Itajaí está no banco (22 centros, 12 com meta), e a área de preparo já contém o histórico da planilha aguardando conferência. Migrations em `supabase/migrations/`; importação em `supabase/import/`.
