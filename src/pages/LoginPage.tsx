...
const LoginPage = () => {
  ...
  // Quem chega pelo link do e-mail já foi atendido antes de a tela montar
  // (src/lib/recovery.ts): aqui só resta abrir na tela certa.
  const recuperacao = recuperacaoEmAndamento();
  const [mode, setMode] = useState<Mode>(recuperacao ? recuperacao.tela : "login");
  ...
  const [alert, setAlert] = useState<{ type: string; msg: string }>(
    // Link velho ou já usado: a tela abre pedindo outro e-mail, com o motivo.
    recuperacao?.tela === "recuperar"
      ? { type: "error", msg: recuperacao.erro }
      : { type: "", msg: "" },
  );
  ...
  async function handleRecuperar(e: React.FormEvent) {
    ...
    try {
      ...
      setAlert({ type: "error", msg: (err as Error)?.message || "Não foi possível enviar o e-mail." });
    }
    setLoading(false);
  }
  ...
  async function handleNovaSenha(e: React.FormEvent) {
    ...
    try {
      ...
      setAlert({ type: "error", msg: (err as Error)?.message || "Não foi possível alterar a senha." });
    }
    setLoading(false);
  }
          ...
          {isLogin && (isSupabase ? (
            <p className="text-center text-xs text-muted-foreground mt-5">
              ...
              <button type="button" onClick={() => switchMode("recuperar")}
                className="font-bold underline underline-offset-2 hover:brightness-110" style={{ color: '#003366' }}>
